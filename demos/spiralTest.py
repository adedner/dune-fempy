...
domain   = dune.grid.cartesianDomain([0,0,0],[2.5,2.5,2.5],[3,3,3])
...
maxOrder = 1
space = dune.fem.space.lagrangehp( gridView, maxOrder=maxOrder,
        storage="petsc" )
spcpm = dune.fem.space.lagrangehp( gridView, maxOrder=maxOrder,
        storage="petsc" )
# space = dune.fem.space.dglegendre( gridView, order=maxOrder,
#         storage="istl" )
# spcpm = dune.fem.space.dglegendre( gridView, order=maxOrder,
#         storage="istl" )
...
def markpm1(element):
    return max(space.localOrder( element ) - 1)
...
while t.value < endTime:
    uh_n.assign(uh)
    vh_n.assign(vh)
    print("starting to solve with |space|:",space.size,"grid size",gridView.size(0))
    info = scheme.solve(target=uh)
    print("memory usage:",maxMemoryUsage("GB"))
    t.value += dt.value
    print("Computed solution at time", t.value,
          "iterations: ", info["linear_iterations"],
          "#Ent: ", gridView.size(0), pMarked )
