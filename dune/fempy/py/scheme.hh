  ...
  namespace FemPy
  {
    ...
    namespace detail
    {
        ...
        -> void_t< decltype( getBCRSMatrix( std::declval< const typename Scheme::LinearOperatorType & >().matrix() ) ) >
      {
        ...
        cls.def( "assemble", [] ( Scheme &self, pybind11::object ubar ) {
            if( isinstance< DiscreteFunction >( ubar ) )
              return getBCRSMatrix( self.assemble( pybind11::cast< const DiscreteFunction & >( ubar ) ).matrix() );
            else
              return getBCRSMatrix( self.assemble( *getVirtualizedGridFunction< GridPart, RangeType >( ubar ).first ).matrix() );
        ...
        -> void_t< decltype( std::declval< const typename Scheme::LinearOperatorType & >().matrix().data() ) >
      {
        ...
        cls.def( "assemble", [] ( Scheme &self, pybind11::object ubar ) {
            if( isinstance< DiscreteFunction >( ubar ) )
              return self.assemble( pybind11::cast< const DiscreteFunction & >( ubar ) ).matrix().data();
            else
              return self.assemble( *getVirtualizedGridFunction< GridPart, RangeType >( ubar ).first ).matrix().data();
          }, pybind11::return_value_policy::reference_internal, "ubar"_a );
      }
