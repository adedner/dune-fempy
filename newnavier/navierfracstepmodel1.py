...
def compute():
    ...
    vtk = grid.sequencedVTK("navierfracstep_model1",
    ...
    def solvestep1():
        ...
        # tau = NamedConstant(spc,name="tau")
        a = (inner(u - as_vector([old_sol[0],old_sol[1]]), v)) * dx + 0.5 * tau * viscosity * b
             # tau * viscosity * inner(grad(u), grad(v))) * dx


        a -= tau * inner(p[0],div(v)) * dx
        a += tau * inner(grad(u_n), outer(v, u_n)) * dx
        a += 0.5 * tau * viscosity * inner(du_n, grad(v)) * dx
    ...
    def solvestep2():
        ...
        aa = (inner(u - as_vector([old_sol[0],old_sol[1]]), v)) * dx + 0.5 * tau * viscosity * b
             # tau * viscosity * inner(grad(u), grad(v))) * dx


        aa -= tau * inner(old_sol[2],div(v)) * dx
        aa += tau * inner(grad(u), outer(v, u)) * dx
        aa += 0.5 * tau * viscosity * inner(du_n, grad(v)) * dx
