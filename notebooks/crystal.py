...
vgrid = create.view("adaptive", grid="ALUConform",
                    constructor=domain, dimgrid=dimDomain)
space = create.space("Lagrange", vgrid, dimrange=dimRange,
                order=order, storage="fem")
initial_gf = create.function("global", vgrid, "initial", order+1, initial)
...
model  = create.model("elliptic", vgrid, equation, coefficients={un:solution_n} )
solverParameters = {
...
def matplot(vgrid, solution, show=range(dimRange)):
    triangulation = vgrid.triangulation()
...
tk = vgrid.writeVTK("crystal", pointdata=[solution],
       celldata=[levelFunction(vgrid), partitionFunction(vgrid)], number=0)

matplot(vgrid,solution, [0])
